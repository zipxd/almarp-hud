...
Citizen.CreateThread(function()
	...
	SendNUIMessage({
		...
		infos = {
			{
				name = 'cash',
				value = '<p style="color:white"><strong>' .. cash .. '</strong></p>'
			},
			{
				name = 'dirtycash',
				value = '<p style="color:white"><strong>' .. dirtycash .. '</strong></p>'
			},
			{
				name = 'bank',
				value = '<p style="color:white"><strong>' .. bank .. '</strong></p>'
			},
			{
				name = 'job',
				value = ('<p style="color:white"><strong>%s</strong> - <strong>%s</strong></p>'):format(xPlayer.job.label, xPlayer.job.grade_label)
			},
			{
				name = 'job2',
				value = ('<p style="color:white"><strong>%s</strong> - <strong>%s</strong></p>'):format(xPlayer.job2.label, xPlayer.job2.grade_label)
			}
		}
	})
end)
...
AddEventHandler('::{korioz#0110}::esx:setJob', function(job)
	SendNUIMessage({
		...
		infos = {
			{
				name = 'job',
				value = ('<p style="color:white"><strong>%s</strong> - <strong>%s</strong></p>'):format(job.label, job.grade_label)
			}
		}
	})
end)
...
AddEventHandler('::{korioz#0110}::esx:setJob2', function(job2)
	SendNUIMessage({
		...
		infos = {
			{
				name = 'job2',
				value = ('<p style="color:white"><strong>%s</strong> - <strong>%s</strong></p>'):format(job2.label, job2.grade_label)
			}
		}
	})
end)
	...
	if account.name == 'cash' then
		SendNUIMessage({
			...
			infos = {
				{
					name = 'cash',
					value = '<p style="color:white"><strong>' .. ESX.Math.GroupDigits(account.money) .. '</strong></p>'
				}
			}
		})
...
AddEventHandler('::{korioz#0110}::tempui:toggleUi', function(value)
	...
	if uiFaded then
		...
		TriggerEvent("MasterLua:RadioUI", true)
	else
		SendNUIMessage({action = 'fadeUi', value = false})
		TriggerEvent("MasterLua:RadioUI", false)
	end
end)
